...
import TodaysTask from "../todaysTaskEmp/page";
...
const VectorEmp = () => {
  ...
  const CompletedTasksList = () => {
    router.push('/completedByEmp');
  };

  const PendingTasksList = () => {
    router.push('/pendingEmp');
  };

  const OverdueTasksList = () => {
    router.push('/overdueByEmployee');
  };
      ...
      <div className="w-full h-screen flex flex-col items-center overflow-x-auto">
        ...
        {selectedLabel && (
          <div className="list-container">
            ...
            {selectedLabel === "Total Tasks" && <ReceivedTasksList />}
            {selectedLabel === "Today Added Tasks" && <TodaysTask />}
          </div>
        )}
      </div>
      <style>{`
