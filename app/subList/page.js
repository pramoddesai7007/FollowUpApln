...
//         if (editedEmployee.phoneNumber.length !== 10) {
...
//                 <th className="px-4 py-2 text-center text-gray-700">Role</th>
//                 <th className="px-4 py-2 text-center text-gray-700">Type</th>
...
//                   <tr key={employee._id}>
//                     <td className="border px-4 py-1 text-center text-sm">{calculateSerialNumber(index)}</td>
//                     <td className="border px-4 py-1 text-center text-sm">{employee.name}</td>
//                     <td className="border px-4 py-1 text-center text-sm">{employee.role}</td>
//                     <td className="border px-4 py-1 text-center text-sm">{employee.type}</td>
//                     <td className="border px-4 py-1 text-center text-sm">{employee.email}</td>
//                     <td className="border px-4 py-1 text-center text-sm">{employee.phoneNumber}</td>
...
import axios from 'axios';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faPenToSquare, faPlus, faTrash, faEye, faMoneyCheckAlt, faFileExcel } from '@fortawesome/free-solid-svg-icons';
import NavSide from '../components/NavSide';
import * as XLSX from 'xlsx';
import jwtDecode from 'jwt-decode';
...
const EmployeeList = () => {
  ...
  const confirmCalculateSalary = async (employeeId) => {
    setError(null);
    try {
      const response = await axios.post(`http://103.159.85.246:4000/api/salary/calculate-salary/${employeeId}`, {
        startDate,
        endDate,
      }, {
        headers: {
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full table-auto mt-1">
            <thead className='bg-orange-600 text-white'>
              <tr>
                <th className="px-1 py-2 text-cente">Sr.No.</th>
                <th className="px-4 py-2 text-cente">Name</th>
              ...
              {employees.length > 0 ? (
                currentEmployees.map((employee, index) => (
                  <tr key={employee._id}>
                    ...
                    <td className="border px-4 py-1 text-center text-sm">{employee.email}</td>
                    <td className="border px-4 py-1 text-center text-sm">{employee.phoneNumber}</td>
                    <td className="border px-4 py-1 flex items-center space-x-3">
                      <FontAwesomeIcon
                        icon={faMoneyCheckAlt}
                        className="text-purple-700 hover:underline cursor-pointer text-lg"
                        onClick={() => confirmCalculateSalary(employee._id)}
                      />
                      <FontAwesomeIcon
                        icon={faEye}
                      ...
                      <FontAwesomeIcon
                        icon={faPenToSquare}
                        className="text-orange-500 hover:underline cursor-pointer text-lg"
                        onClick={() => handleEditClick(employee._id)}
                      />
                      <FontAwesomeIcon
                        icon={faTrash}
                        className="text-red-500 hover:underline cursor-pointer text-lg"
                        onClick={() => handleDeleteClick(employee._id)}
                      />
                    </td>
                  </tr>
        ...
        {isSalaryModalOpen && (
          ...
          >
            <div className="modal-container bg-white w-72 md:w-96 rounded shadow-lg">
              ...
              <div className="p-6 text-center">
                <h3 className="mb-5 text-lg font-semibold text-gray-800 dark:text-gray-400">Salary Details</h3>
                {totalSalary !== null && (
                  <div className="text-center mt-4">
                    <p className="text-lg font-semibold">
                      Total Salary for {getCurrentMonthName()}: ₹ {totalSalary.toFixed(2)}
                    </p>
                  </div>
                )}
                <button
