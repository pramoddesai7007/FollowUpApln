...
const ReceivedTaskList = () => {
  ...
  const handleDateSearch = async () => {
    ...
    // Fetch tasks based on date range from the server
    try {
                ...
                {tasks.length > 0 ? (
                  currentTasks.map((task, index) => (
                    <tr key={task._id} className='hover:bg-gray-100 text-sm cursor-pointer'>
                      <td className="px-2 py-1 whitespace-nowrap text-center border font-semibold">{calculateSerialNumber(index)}</td>
                      <td className="px-2 py-1 whitespace-nowrap border  text-center font-bold text-orange-700">{task.title}</td>
                      <td className="px-6 py-1 whitespace-nowrap border  text-center">
                        <span className='px-4 py-1 bg-blue-200 text-blue-800 rounded-full text-sm font-bold'>{task.status}</span></td>
                      <td className="px-6 py-1 whitespace-nowrap border  text-center">{task.startDate}</td>
                      <td className="px-6 py-1 whitespace-nowrap border  text-center">{task.deadlineDate}</td>
                      <td className="px-5 py-1 whitespace-nowrap border">
                        <FontAwesomeIcon
                          icon={faEye}
