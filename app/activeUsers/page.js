...
const UserList = () => {
    ...
    useEffect(() => {
        const fetchAssignedEmployees = async () => {
          try {
            const response = await axios.get('http://103.159.85.246:4000/api/task/tasks/active-users'); // Assuming your API endpoint is '/api/tasks/assigned'
                        ...
                        <tbody>
                            {employees.length > 0 ? (
                                currentEmployees.map((employee, index) => (
                                    <tr key={employee._id} className='text-sm cursor-pointer hover:bg-gray-100'>
                                        <td className="border px-4 py-1.5 text-center">{calculateSerialNumber(index)}</td>
                                        <td className="border px-4 py-1.5 text-center">{employee.name}</td>
                                        <td className="border px-4 py-1.5 text-center">{employee.email}</td>
                                        <td className="border px-4 py-1.5 text-center">{employee.phoneNumber}</td>
                                        <td className="border px-4 py-1.5 text-center">{employee.adminCompanyName}</td>
